...
void DisplayError(S32 status, const char* funcName, const BRDCHAR* cmd_str)
{
	...
	{
    case (S32)BRDerr_OK:
        BRDC_snprintf(msg, sizeof(msg), _BRDC("%s - %s: BRDerr_OK\n"), func_name, cmd_str);
		break;
    case (S32)BRDerr_BAD_MODE:
        BRDC_snprintf(msg, sizeof(msg), _BRDC("%s - %s: BRDerr_BAD_MODE\n"), func_name, cmd_str);
		break;
    case (S32)BRDerr_INSUFFICIENT_SERVICES:
        BRDC_snprintf(msg, sizeof(msg), _BRDC("%s - %s: BRDerr_INSUFFICIENT_SERVICES\n"), func_name, cmd_str);
		break;
    case (S32)BRDerr_BAD_PARAMETER:
        BRDC_snprintf(msg, sizeof(msg), _BRDC("%s - %s: BRDerr_BAD_PARAMETER\n"), func_name, cmd_str);
		break;
    case (S32)BRDerr_BUFFER_TOO_SMALL:
        BRDC_snprintf(msg, sizeof(msg), _BRDC("%s - %s: BRDerr_BUFFER_TOO_SMALL\n"), func_name, cmd_str);
		break;
    case (S32)BRDerr_WAIT_TIMEOUT:
        BRDC_snprintf(msg, sizeof(msg), _BRDC("%s - %s: BRDerr_WAIT_TIMEOUT\n"), func_name, cmd_str);
		break;
	default:
        BRDC_snprintf(msg, sizeof(msg), _BRDC("%s - %s: Unknown error, status = %8X\n"), func_name, cmd_str, real_status);
		break;
	}
    BRDC_printf(_BRDC("\n%s"), msg);
}
