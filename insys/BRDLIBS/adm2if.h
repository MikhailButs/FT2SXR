...
typedef union _ADM2IF_FORMAT {
  U32 AsWhole;	// Register as a Whole Word
  struct {			// Register as Bit Pattern
   ULONG Pack	: 8,	// 0 - 16 bit, 1 - 8 bit, 2 - 16 bit, 3 - 24 bit, 4 - 32 bit
		 Code	: 4,	// 0 - two's complement, 1 - floating point, 2 - straight binary, 7 - Gray code
...
typedef union _TETRAD_IDRES {
	...
	struct { // Status Register as Bit Pattern
		USHORT	Res0		: 4, // Reserved
				...
				LvlFlagFifo	: 1, // 1 - можно устанавливать уровень срабатывания флагов FIFO
				Res1		: 9; // Reserved
	} ByBits;
...
typedef enum _ADM2IF_NUM_DIRECT_REGS {
	...
	ADM2IFnr_STDDELAY		= 0x2F0,	// write/read/reset delay for ADC/DAC channels / external start / SYNX
} ADM2IF_NUM_DIRECT_REGS;

#include "module.h"

inline ULONG RegRead(CModule* pModule, ULONG srvMainIdx, ULONG tetr, ULONG reg, ULONG cmd)
{
...
inline ULONG WaitCmdReady(CModule* pModule, ULONG srvMainIdx, ULONG main_tetr, long msTimeout)
{
	DEVS_CMD_Reg param;
