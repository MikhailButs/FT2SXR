...
int WaitEvent( KEVENT * event, u32 timeout )
{
    ...
    //printk("<0>%s(): timeout value %d\n", __FUNCTION__, timeout);

#ifdef DZYTOOLS_2_4_X
    status = interruptible_sleep_on_timeout( &event->m_wq, msecs_to_jiffies(timeout) );
#else
    status = wait_event_interruptible_timeout( event->m_wq, atomic_read(&event->m_flag), msecs_to_jiffies(timeout) );
#endif
